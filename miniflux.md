...
│                                                             │
│  ┌──────────┐  ┌─────────────┐  ┌─────────────────────────┐│
│  │  首页    │  │  订阅中心   │  │  Notebook 详情页        ││
│  │ /home    │  │  /feeds     │  │  /notebook/:id          ││
│  │          │  │             │  │                         ││
│  │ 今日简报 │──│ 分类列表    │  │  来源文章列表           ││
│  │ 卡片入口 │  │ Feed 管理   │  │  (含 rss_entry 类型)    ││
│  │          │  │ Entry 浏览  │  │                         ││
│  └──────────┘  └─────────────┘  └─────────────────────────┘│
│                       │                     ▲               │
│                       │    "导入到笔记本"   │               │
...
在现有首页 `"最近打开"` 区域上方，新增一个可收起的 **"今日简报"** 卡片：
...
- 展示未读文章总数和更新的 Feed 数量。
- **查看简报**：跳转到订阅中心的简报视图。
- **管理订阅**：跳转到订阅中心的 Feed 管理视图。
- 若无任何订阅，显示引导卡片："添加您的第一个订阅源"。

#### 3.2.2 订阅中心页 `/feeds`

这是 RSS 功能的核心页面，采用与 Notebook 详情页一致的三栏布局风格：
...
- **简报视图（默认）**：按时间倒序展示近 24 小时新文章，每篇显示：
  ```
  ┌─────────────────────────────────────────┐
  │ [Feed favicon] Feed 名称    2 小时前    │
  │ 文章标题                                │
  │ AI 摘要：这篇文章讨论了...（1-2 句）     │
  │               [导入笔记本] [标记已读] ☆  │
  └─────────────────────────────────────────┘
...
- **Feed 视图**：点击左栏某个 Feed 后，列出该 Feed 的所有文章（分页加载）。
...
#### 3.2.4 Notebook 详情页集成

在现有 **添加来源** 弹窗（`AddSourceModal`）的入口中，新增 **"从订阅源导入"** 选项：
...
点击 "从订阅源导入" 后，弹出 RSS 文章选择面板，展示用户的订阅文章列表，支持搜索和筛选，勾选后批量导入到当前 Notebook。
...
```
设置
[语言] [外观] [聊天模型] [Embedding] [搜索] [RSS 订阅] [账户]
...
| 入口位置 | 形式 | 说明 |
|---------|------|------|
| 首页顶栏 | RSS 图标按钮（📡） | 位于设置齿轮旁边，点击进入订阅中心 |
| 首页内容区 | 今日简报卡片 | 位于"最近打开"上方 |
| Notebook 添加来源弹窗 | "从订阅源导入"按钮 | 在搜索和文件上传之间 |
| 设置弹窗 | "RSS 订阅" Tab | 配置 Miniflux 和 RSSHub 连接 |
...
在简报视图中，每篇 RSS Entry 需要一个 1-2 句话的快速摘要。此摘要与 Notebook 内的深度摘要不同：

| 维度 | Entry 快速摘要 | Notebook 深度摘要 |
|------|---------------|------------------|
| 触发时机 | 简报生成时批量处理 | 用户点击文章摘要按钮时 |
| 长度 | 1-2 句（≤ 80 字） | 完整段落（300-800 字） |
| 模型 | Lite LLM（低成本） | 用户配置的聊天模型 |
| 缓存 | 存储在 Miniflux Entry 的本地缓存中 | 存储在 `summary_caches` 表 |
| 输入 | Entry 的 content HTML（截取前 2000 字） | Article 的 clean_markdown 全文 |

### 7.2 简报生成流程

由 Scheduler 在用户设定的时间（默认 08:00）触发：

1. 查询用户的 Miniflux 未读 Entry（过去 24 小时）。
2. 对每篇 Entry，调用 Lite LLM 生成快速摘要（并发限制 5）。
3. 将结果写入 `rss_digests` 表。
4. 更新首页简报计数。
...
```jsx
// App.jsx
<Route path="/feeds" element={<RequireSession><FeedsPage /></RequireSession>} />
```
...
| 文件路径 | 组件 | 说明 |
|---------|------|------|
| `pages/FeedsPage.jsx` | FeedsPage | 订阅中心主页面 |
| `components/FeedSidebar.jsx` | FeedSidebar | 左栏分类/Feed 列表 |
| `components/EntryList.jsx` | EntryList | 中栏文章列表 |
| `components/EntryPreview.jsx` | EntryPreview | 右栏文章预览 |
| `components/AddFeedModal.jsx` | AddFeedModal | 添加订阅源弹窗 |
| `components/DigestCard.jsx` | DigestCard | 首页今日简报卡片 |
...
**目标**：用户可以添加、管理 RSS 订阅源，浏览文章，将文章导入到 Notebook。

- 后端 `modules/feeds/` 模块：MinifluxClient、Feed CRUD API、Entry 查询 API。
- 数据库：`rss_feeds` 表迁移。
- Docker Compose：Miniflux 服务集成。
- 前端：FeedsPage 三栏布局、AddFeedModal、Feed/Entry 列表渲染。
- 前端：AddSourceModal 新增"从订阅源导入"入口。
- 导入桥接：`import-rss` 端点，复用现有 Ingest Pipeline。
...
- 前端：DigestCard 首页组件、简报视图。

### Phase 3：RSSHub 集成与体验优化
...
| `frontend/src/App.jsx` | 新增 `/feeds` 路由 | 低——一行 Route |
