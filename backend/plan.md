...
所以我建议这一域的职责收敛成一句话：**用户可以自带生成模型配置，但系统内部的搜索 provider、嵌入模型、解析链路不跟着用户设置走。** 也就是说，BYOK 只作用于 `summary/chat` 的生成模型；`search` 用平台持有的 Exa Key；embedding 用平台固定模型。这样做的原因不是“偷懒”，而是避免把搜索质量、索引一致性、向量维度、重建索引成本都暴露给用户设置。当前前端设置里本来也没有 embedding 相关字段，这样切分最自然。
...
我建议的运行拓扑不是复杂微服务，而是**单代码仓库、三个运行单元**：`api`、`worker`、`scheduler`。`api` 负责所有 HTTP；`worker` 负责 deep search 和 ingest 等长任务；`scheduler` 负责清理过期搜索会话、重发未发布任务、做缓存清理。外围依赖是 PostgreSQL、Redis、RocketMQ、对象存储、Exa、LLM Provider、OTel Collector、Langfuse、Grafana/Loki/Tempo。这个形态足够轻，又不会把长任务塞进请求线程里。
...
具体来说，`search_result` 和 `url` 走 `Exa Contents -> Trafilatura fallback`。`text` 直接做 markdown 归一化。`file` 走 `MarkItDown -> Docling fallback`。一旦拿到候选正文，后面就统一走：质量评分、必要时 LLM markdown fallback、clean、TOC、content_hash、article_retrieval_text、article_vector、heading-aware chunking、chunk_vector、索引替换。

这里有一个我认为非常关键的细节：**preview_markdown 和 clean_markdown 是两个不同层级的内容对象。** `preview_markdown` 来自 search 阶段的 highlights，目的是在 article 还没 ready 时给 UI 一个可读占位。`clean_markdown` 才是解析完成后的标准正文，也是 summary/chat/retrieval 的真正输入。当前前端不需要感知 processing 状态，但 notebook detail 完全可以通过 `content = clean_markdown or preview_markdown or ""` 的策略，让用户在导入后立即看到内容。
...
### 2.8 Generation 子系统：BYOK 只影响生成，不影响索引

当前设置页里有 `modelProvider`、`modelName`、`apiUrl`、`apiKey`，说明用户生成模型是可配置的。我建议把生成模型解析做成单独组件 `GenerationModelResolver`。它的规则只有两条：用户设置了 BYOK，就用用户配置；没设置，就回退到平台默认模型。这个 resolver 只给 `summary` 和 `chat` 用。

embedding 则必须平台托管，不能跟着用户设置走。原因很直接：如果 embedding 也跟用户模型走，那你会遇到向量维度不一致、历史内容无法比较、需要按用户重建索引、search quality 不可控等一系列问题。把 generation 和 embedding 分开，是这个系统长期可维护的关键之一。
...
### 2.12 可观测性：OTel 打全链路，Langfuse看 LLM

OpenTelemetry Python 官方提供了 FastAPI 和 SQLAlchemy 的 instrumentation；Langfuse 明确支持把 retrieval、embedding、API 调用等都纳入 trace，并且基于 OpenTelemetry。换句话说，你完全可以把系统监控分两层：OTel 负责全链路基础遥测，Langfuse 负责 LLM/AI 专项 trace。([OpenTelemetry Python Contrib](https://opentelemetry-python-contrib.readthedocs.io/en/latest/instrumentation/fastapi/fastapi.html))
...
| `modules/ai/summary_service.py`     | `get_summary(user_id, notebook_id, article_id, req)`           | 版本化缓存 + 模型调用                                       |
| `modules/ai/chat_service.py`        | `reply(user_id, notebook_id, req)`                             | 会话持久化、路由、检索、prompt、模型调用                    |
...
这一阶段不是“先起个 FastAPI 项目”。你要一次把这些都起起来：FastAPI、SQLAlchemy 2.0、Alembic、PostgreSQL、Redis、RocketMQ、对象存储接口、OTel、Langfuse、结构化日志。history 里已经把底座建议成 `FastAPI + Pydantic + SQLAlchemy + PostgreSQL + Redis + worker + OTel + Langfuse` 这一套，这个方向没问题；我这里只是把 RocketMQ 和 Exa provider 层也直接纳进来了。([Langfuse](https://langfuse.com/docs))

这一步要同时产出 `infra/db/session.py`、`infra/mq/*`、`infra/providers/exa/*`、`infra/telemetry/*`、`infra/storage/object_store.py`，以及所有核心 env 配置项。至少包括：`EXA_API_KEY`、`APP_DEFAULT_MODEL_PROVIDER`、`APP_DEFAULT_MODEL_NAME`、`APP_DEFAULT_MODEL_API_URL`、`EMBEDDING_PROVIDER`、`EMBEDDING_MODEL`、`ROCKETMQ_NAMESRV_ADDR`、`SEARCH_INLINE_DEADLINE_MS`、`SUMMARY_CACHE_TTL_DAYS`。
...
但这次要特别注意 `settings`。数据库不建 `user_settings` 表，而是 `users.settings_json + llm_api_key_ciphertext + llm_api_key_last4`。`GET /settings` 由 service 层聚合生成默认值并做脱敏；`PUT /settings` 按 patch 合并。与此同时，`PATCH /account/profile` 只改 `users.name`，绝不把 `username` 落进 settings JSON。这个细节如果一开始做错，后面设置和资料会永远缠在一起。
...
### 5.6 步骤 5：建统一 ingest pipeline，把 search/url/text/file 都收口

这一步的重点不是“多支持几种格式”，而是**用同一条 pipeline 统一所有输入**。history 里已经把统一链路写成 `raw input -> deterministic parse -> AI markdown fallback -> clean markdown -> article embedding -> heading-aware chunking -> chunk embedding -> index`，我建议完全保留这个骨架，只把 provider 和 parser 路由补实。

这一阶段至少要产出以下组件：`ParserRouter`、`ParseQualityScorer`、`MarkdownCleaner`、`TOCExtractor`、`RetrievalTextBuilder`、`HeadingAwareChunker`、`Embedder`、`IndexUpdater`。验收标准不是“代码跑通”，而是：URL、text、file 三类输入都能最终产出 `clean_markdown / toc_json / content_hash / article_retrieval_text / article_vector / chunks`。
...
`summary` 的服务本身不复杂，复杂的是缓存键和模型解析。你这一步要写 `GenerationModelResolver`、`SummaryCacheRepo`、`SummaryService` 三块。关键点是：cache key 带 `content_hash`；如果 article 还没 ready，就返回 `409 article_not_ready`；如果用户没配 BYOK 又没有平台默认生成模型，就返回 `422 model_config_required`。
...
这一阶段你至少要打通三类面板：search、ingest、LLM。search 面板看 inline 成功率、降级到 async 的比例、provider 耗时、search result 导入率；ingest 面板看 parse 成功率、fallback 命中率、平均 chunk 数、失败类型；LLM 面板看 summary/chat 延迟、token、成本、错误率。Langfuse 可以直接吃 LLM、retrieval、embedding 和会话 session；FastAPI 和 SQLAlchemy 侧用 OTel 自动埋点即可。([Langfuse](https://langfuse.com/docs))
...
第六，BYOK 只影响生成模型，embedding 与 search provider 都平台托管。
