...
- **route accuracy**（问题被分到正确 route 的比例）
- **evidence recall@k**（前 k 条证据中，命中 gold evidence 的数量 ÷ 该问 gold evidence 总数）
- **citation precision**（模型给出的引用中，正确/相关引用所占比例）
- **answer faithfulness**（答案与证据一致、不捏造的程度）
- **scope correctness**（该用/不该用 notebook 或 article 时判断正确的比例）
- **recommendation nDCG@5**（推荐列表前 5 条按位置折损后的相关性/相似度综合得分）
- **notebook synthesis coverage**（回答对 notebook 内多篇内容的综合引用/覆盖情况）
