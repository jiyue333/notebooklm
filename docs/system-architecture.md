...
本文档基于当前仓库代码整理，目标是回答三类问题：

1. 系统里有哪些核心组件，它们如何调用。
2. 搜索、导入、异步 job、聊天、摘要这些关键链路如何流转。
3. 数据落在哪里，什么时候是同步流程，什么时候会进入异步流程。
...
| API 层 | FastAPI | `backend/app/main.py` | 路由注册、依赖注入、中间件、生命周期、统一观测初始化 | Postgres、Redis、RocketMQ、对象存储、LLM/Search provider |
...
| 搜索 | Search 模块 | `backend/app/modules/search/*` | Exa 搜索、搜索会话、搜索结果、来源导入、手动来源创建、文件上传 | Exa、Postgres、对象存储、RocketMQ |
...
| 异步任务 | Jobs 模块 | `backend/app/modules/jobs/*` | Job 建模、发布、重发、inline fallback | RocketMQ、Worker、Scheduler |
| Worker | 异步消费者 | `backend/app/workers/run_worker.py` | 消费 `search_deep`、`article_ingest`、`article_reindex` | RocketMQ、Postgres、Exa、LLM、对象存储 |
| Scheduler | 定时任务 | `backend/app/workers/run_scheduler.py`, `backend/app/modules/jobs/scheduler.py` | 重发 pending job、清理失败 job、清理过期会话/摘要缓存 | Postgres、RocketMQ |
...
| 队列 | RocketMQ | `backend/app/infra/mq/*`, `docker/rocketmq/*` | Job 传递、消费确认、失败重试 | API、Worker、Scheduler |
...
| API | RocketMQ | producer | 发布 `search_deep`、`article_ingest`、`article_reindex` |
| Worker | RocketMQ | consumer | 消费异步任务 |
| Worker | Exa / Trafilatura / 文件解析器 | HTTP / 本地解析 | 获取正文、清洗正文 |
| Worker | LLM provider | HTTP / LangChain | embedding、必要时正文 fallback |
| Scheduler | Postgres + RocketMQ | SQL + producer | 重发 pending job、清理历史数据 |
...
flowchart LR
    ...
    B --> C["publish_jobs()"]
    C -->|"RocketMQ 发送成功"| D["mark_job_queued()"]
    C -->|"RocketMQ 异常"| E["run_job_inline() 或保持 pending_publish"]
